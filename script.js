...
function setupTables() {
    const pCount = parseInt(document.getElementById('numProcesses').value) || 5;
    const rCount = parseInt(document.getElementById('numResources').value) || 4;
    resetUI();
    ...
    for (let j = 0; j < rCount; j++) {
        requestContainer.innerHTML += `
            <input type="number" class="request-val" placeholder="${String.fromCharCode(65+j)}" 
            value="0" min="0" style="width:50px;" onchange="validateRequestInput(this)">`;
    }
...
function validateRequestInput(el) {
    ...
    // Note: Does not call resetUI(), so Need Matrix stays visible
}
...
function handleRequest() {
    ...
    for(let j=0; j<rCount; j++) {
        if (request[j] > need[j]) { 
            document.getElementById('request-result').innerHTML = `<span class="log-fail">Denied: Request > Need</span>`; 
            return; 
        }
        if (request[j] > avail[j]) { 
            document.getElementById('request-result').innerHTML = `<span class="log-fail">Denied: Request > Available</span>`; 
            return; 
        }
    }
...
function loadSampleData() {
    ...
    setTimeout(() => {
        ...
        for(let i=0; i<5; i++) {
            ...
            for(let j=0; j<4; j++) { 
                aIn[j].value = aData[i][j]; 
                mIn[j].value = mData[i][j]; 
            }
        }
        alert("Sample data loaded. Click Step 1 to begin.");
    }, 150);
}